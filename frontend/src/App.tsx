...
const Dashboard: React.FC = () => {
  ...
  const fetchVehicles = useCallback(async () => {
    ...
    try {
      if (!isAuthenticated) {
        // If unauthenticated, fetch mock/public vehicles or attempt search
        // Note: Protected endpoints expect auth token, so if not logged in we fall back gracefully
      }
      const data = await vehiclesAPI.getAll();
      setVehicles(data);
    } catch (err: any) {
      if (err.response?.status === 401) {
        // Unauthenticated request, prompt login or keep empty
      } else {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-slate-900 to-cyan-950/60 border border-slate-800 p-8 mb-8 shadow-2xl">
          ...
          <div className="relative z-10 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-950/80 border border-cyan-800/50 text-cyan-400 text-xs font-bold uppercase tracking-wider mb-4">
              <Sparkles className="w-3.5 h-3.5" /> Full-Stack TDD Kata Platform
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight leading-tight">
              Premium Car Dealership & Inventory System
            </h1>
            <p className="text-slate-400 text-sm mt-3 leading-relaxed">
              Explore high-performance electric, sports, and luxury vehicles. Test RESTful endpoints, token authentication, purchase transactions, and admin restock logic in real-time.
            </p>
          </div>
        </div>
      ...
      <footer className="border-t border-slate-900 bg-slate-950/80 py-8 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p>© 2026 Apex Motors TDD Kata System. Built with Node.js, Express, SQLite, React & Tailwind.</p>
          <div className="flex items-center gap-4 text-slate-400">
            <span className="flex items-center gap-1"><ShieldCheck className="w-4 h-4 text-emerald-400" /> JWT Protected</span>
